...
- Added `restoreDomain` and `getDomainRestore` to restore a domain and retrieve a domain restore. (#182)
- Added `batchChangeZoneRecords` to create, update, and delete zone records in one atomic request. (#185)
- Added `dnsAnalytics->query` to query DNS Analytics data. The API is in Public Beta. (#186)

## 7.1.0 - 2026-09-17

### Deprecated

- Deprecated `parentId` in `ZoneRecord`. The value is always null, and the property will be removed in the next major version. (#180)
...
- Added `new_account_identifier` option to `initiatePush` for initiating domain pushes by account identifier. (#161)
- Added `name` to `Account`. (#161)
- Added support for trustee service in domain transfers. (#163)

### Deprecated

- Deprecated `new_account_email` option in `initiatePush`. Use `new_account_identifier` instead. (#161)

## 6.2.0 - 2026-03-23

### Added

- Added support for trustee service. (#158)
