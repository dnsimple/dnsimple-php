<?php

namespace Dnsimple\Service;

use Dnsimple\Response;
use Dnsimple\Struct\Certificate;
use Dnsimple\Struct\CertificateDownload;
use Dnsimple\Struct\CertificatePrivateKey;
use Dnsimple\Struct\CertificatePurchase;
use Dnsimple\Struct\CertificateRenewal;

class CertificatesTest extends ServiceTestCase
{

    protected function setUp(): void
    {
        parent::setUp();
        $this->service = new Certificates($this->client);
    }

    public function testListCertificates()
    {
        $this->mockResponseWith("listCertificates/success");

        $response = $this->service->listCertificates(1010, "example.com");
        self::assertInstanceOf(Response::class, $response);
        self::assertEquals(200, $response->getStatusCode());

        $data = $response->getData();
        self::assertCount(2, $data);

        $certificate = $data[0];
        self::assertInstanceOf(Certificate::class, $certificate);
        self::assertEquals(101973, $certificate->id);
    }

    public function testListCertificatesSupportsSorting()
    {
        $this->mockResponseWith("listCertificates/success");
        $this->service->listCertificates(1010, "example.com", ["sort"=>"id:asc,common_name:desc,expiration:asc"]);

        self::assertEquals("sort=id%3Aasc%2Ccommon_name%3Adesc%2Cexpiration%3Aasc", $this->mockHandler->getLastRequest()->getUri()->getQuery());
    }


    public function testListCertificatesHasPaginationObject()
    {
        $this->mockResponseWith("listCertificates/success");
        $response = $this->service->listCertificates(1010, 1);
        $pagination = $response->getPagination();

        self::assertEquals(1, $pagination->currentPage);
        self::assertEquals(30, $pagination->perPage);
        self::assertEquals(2, $pagination->totalEntries);
        self::assertEquals(1, $pagination->totalPages);
    }

    public function testListCertificatesSupportsPagination()
    {
        $this->mockResponseWith("listCertificates/success");
        $this->service->listCertificates(1010, 1, ["page" => 1, "per_page" => 4]);

        self::assertEquals("page=1&per_page=4", $this->mockHandler->getLastRequest()->getUri()->getQuery());
    }

    public function testGetCertificate()
    {
        $this->mockResponseWith("getCertificate/success");

        $certificate = $this->service->getCertificate(1010, "example.com", 101967)->getData();

        self::assertEquals(101967, $certificate->id);
        self::assertEquals(289333, $certificate->domainId);
        self::assertEquals(2511, $certificate->contactId);
        self::assertEquals("www", $certificate->name);
        self::assertEquals("www.bingo.pizza", $certificate->commonName);
        self::assertEquals(1, $certificate->years);
        self::assertEquals("-----BEGIN CERTIFICATE REQUEST-----\\n-----END CERTIFICATE REQUEST-----\n", $certificate->csr);
        self::assertEquals("issued", $certificate->state);
        self::assertFalse($certificate->autoRenew);
        self::assertEmpty($certificate->alternateNames);
        self::assertEquals("letsencrypt", $certificate->authorityIdentifier);
        self::assertEquals("2020-06-18T18:54:17Z", $certificate->createdAt);
        self::assertEquals("2020-06-18T19:10:14Z", $certificate->updatedAt);
        self::assertEquals("2020-09-16T18:10:13Z", $certificate->expiresAt);
    }

    public function testDownloadCertificate()
    {
        $this->mockResponseWith("downloadCertificate/success");
        $certificate = $this->service->downloadCertificate(1010, "example.com", 101967)->getData();

        self::assertInstanceOf(CertificateDownload::class, $certificate);
        self::assertEquals("-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n", $certificate->server);
        self::assertNull($certificate->root);
        self::assertEquals(["-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----"], $certificate->chain);
    }

    public function testGetCertificatePrivateKey()
    {
        $this->mockResponseWith("getCertificatePrivateKey/success");
        $privateKey = $this->service->getCertificatePrivateKey(1010, "example.com", 101967)->getData();

        self::assertInstanceOf(CertificatePrivateKey::class, $privateKey);
        self::assertEquals("-----BEGIN RSA PRIVATE KEY-----\\nU8blTH/uqpR2pSC6whzJ/lnSdqHUqhyp00000000000000000000\n-----END RSA PRIVATE KEY-----\n", $privateKey->privateKey);
    }

    public function testPurchaseLetsEncryptCertificate()
    {
        $this->mockResponseWith("purchaseLetsEncryptCertificate/success");

        $attributes = [
            "contact_id" => 42
        ];
        $certificatePurchase = $this->service->purchaseLetsEncryptCertificate(1010, "example.com", $attributes)->getData();

        self::assertInstanceOf(CertificatePurchase::class, $certificatePurchase);
        self::assertEquals(101967, $certificatePurchase->id);
        self::assertEquals(101967, $certificatePurchase->certificateId);
        self::assertEquals("new", $certificatePurchase->state);
        self::assertFalse($certificatePurchase->autoRenew);
        self::assertEquals("2020-06-18T18:54:17Z", $certificatePurchase->createdAt);
        self::assertEquals("2020-06-18T18:54:17Z", $certificatePurchase->updatedAt);
    }

    public function testIssueLetsencryptCertificate()
    {
        $this->mockResponseWith("issueLetsencryptCertificate/success");

        $certificate = $this->service->issueLetsencryptCertificate(1010, "example.com", 42)->getData();

        self::assertInstanceOf(Certificate::class, $certificate);
    }

    public function testPurchaseLetsencryptCertificateRenewal()
    {
        $this->mockResponseWith("purchaseRenewalLetsencryptCertificate/success");

        $attributes = [
            "auto_renew" => false
        ];
        $renewal = $this->service->purchaseLetsencryptCertificateRenewal(1010, "example.com", 101967, $attributes)->getData();

        self::assertInstanceOf(CertificateRenewal::class, $renewal);
        self::assertEquals(65082, $renewal->id);
        self::assertEquals(101967, $renewal->oldCertificateId);
        self::assertEquals(101972, $renewal->newCertificateId);
        self::assertEquals("new", $renewal->state);
        self::assertFalse($renewal->autoRenew);
        self::assertEquals("2020-06-18T19:56:20Z", $renewal->createdAt);
        self::assertEquals("2020-06-18T19:56:20Z", $renewal->updatedAt);
    }

    public function testIssueLetsencryptCertificateRenewal()
    {
        $this->mockResponseWith("issueRenewalLetsencryptCertificate/success");

        $certificate = $this->service->issueLetsencryptCertificateRenewal(1010, "example.com", 42, 44)->getData();

        self::assertInstanceOf(Certificate::class, $certificate);
        self::assertEquals("requesting", $certificate->state);

    }


}
