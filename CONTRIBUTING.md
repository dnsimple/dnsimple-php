...
Add new entries to the `## Unreleased` section at the top of `CHANGELOG.md`. When a change requires a major version, append `(requires major version)` to its entry, for example `- Drop support for PHP < X.Y (requires major version)`. The release process uses this tag to select the next version.
